#include <windows.h>
/*
 * DC4 Patch DLL (Injection Version)
 *
...
public:
  void Init() {
    ...
    m_iniPath = exeDir + L"\\DC4Patch.ini";
...
// Function pointer - Detours will update this to the trampoline
#ifdef GAME_DC4PH
static void(__cdecl *Real_BacklogFunc)() = (void(__cdecl *)())0x00405340;
#else
static void(__cdecl *Real_BacklogFunc)() = (void(__cdecl *)())0x00404EE0;
#endif
...
static HWND WINAPI Hook_CreateWindowExW(DWORD dwExStyle, LPCWSTR lpClassName, LPCWSTR lpWindowName, DWORD dwStyle, int X, int Y, int nWidth, int nHeight, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam) {
    if (lpWindowName && wcsstr(lpWindowName, L"D.C.") != NULL) {
        lpWindowName = L"D.C. II ~Da Capo II~ Dearest Marriage";
    }
...
static BOOL WINAPI Hook_SetWindowTextA(HWND hWnd, LPCSTR lpString) {
    if (lpString && strstr(lpString, "D.C.") != NULL) {
        lpString = "D.C. II ~Da Capo II~ Dearest Marriage";
    }
...
static BOOL WINAPI Hook_SetWindowTextW(HWND hWnd, LPCWSTR lpString) {
    if (lpString && wcsstr(lpString, L"D.C.") != NULL) {
        lpString = L"D.C. II ~Da Capo II~ Dearest Marriage";
    }
...
static INT_PTR CALLBACK SettingsDialogProc(HWND hwndDlg, UINT uMsg,
  ...
  case WM_INITDIALOG: {
    ...
    SetDlgItemInt(hwndDlg, IDC_EDIT_DIALOGUE_SIZE, g_fontManager.GetDialogueFontSize(), TRUE);
    SetDlgItemInt(hwndDlg, IDC_EDIT_BACKLOG_SIZE, g_fontManager.GetBacklogFontSize(), TRUE);
  ...
  case WM_COMMAND: {
    ...
    case IDC_BTN_CHANGE_DIALOGUE_FONT: {
      ...
      if (ChooseFontW(&cf)) {
        ...
        MessageBoxW(hwndDlg, L"Dialogue font updated!", L"DC4 Patch",
    ...
    case IDC_BTN_CHANGE_BACKLOG_FONT: {
      ...
      if (ChooseFontW(&cf)) {
        ...
        MessageBoxW(hwndDlg, L"Backlog font updated!", L"DC4 Patch",
                    MB_OK | MB_ICONINFORMATION);
      }
      return TRUE;
    }
    ...
    case IDC_BTN_CHANGE_BACKLOG_NAME_FONT: {
      ...
      if (ChooseFontW(&cf)) {
        ...
        MessageBoxW(hwndDlg, L"Backlog Character Name font updated!",
                    L"DC4 Patch", MB_OK | MB_ICONINFORMATION);
      }
      return TRUE;
    }
    ...
    case IDC_BTN_OK: {
      ...
      // Save sizing and spacing
      int diagSize = GetDlgItemInt(hwndDlg, IDC_EDIT_DIALOGUE_SIZE, NULL, TRUE);
      int backSize = GetDlgItemInt(hwndDlg, IDC_EDIT_BACKLOG_SIZE, NULL, TRUE);
      int diagSpace = GetDlgItemInt(hwndDlg, IDC_EDIT_DIALOGUE_LINE_SPACING, NULL, TRUE);
      int backSpace = GetDlgItemInt(hwndDlg, IDC_EDIT_SPACING, NULL, TRUE);

      // We need to trigger Font rebuilding by invoking Set...Font.
      g_fontManager.SetDialogueFont(g_fontManager.GetDialogueFontName(), diagSize);
      g_fontManager.SetBacklogFont(g_fontManager.GetBacklogFontName(), backSize);
      
      // Update line spacings
      g_fontManager.SetBacklogOffsets(
          g_fontManager.GetBacklogXOffset(),
          g_fontManager.GetBacklogYOffset(),
          backSpace,
          g_fontManager.GetBacklogNameXOffset(),
          g_fontManager.GetBacklogNameYOffset(),
          g_fontManager.GetBacklogNameSpacing(),
          g_fontManager.GetBacklogDialogSpacing(),
          diagSpace
      );
      ...
      if (anyChanged) {
        ...
        MessageBoxW(hwndDlg,
                    ...
                    L"DC4 Patch", MB_OK | MB_ICONWARNING);
      }
...
static void ShowSettingsDialog() {
  ...
  *pw++ = 0; // Class
  ...
  const WCHAR *dlgTitle = L"DC4 Patch Settings";
...
static HWND WINAPI Hook_CreateWindowExA(DWORD dwExStyle, LPCSTR lpClassName,
  ...
  if (lpWindowName && strstr(lpWindowName, "D.C.") != NULL) {
      lpWindowName = "D.C. II ~Da Capo II~ Dearest Marriage";
  }
  ...
  if (uMsg == WM_SYSCOMMAND) {
    ...
    if (wParam == 0x114514) {
      ...
      if (ChooseFontW(&cf)) {
        int height = lf.lfHeight;
        g_fontManager.SetDialogueFont(lf.lfFaceName, height);
        MessageBoxW(hWnd, L"Font updated!", L"DC4 Patch",
                    MB_OK | MB_ICONINFORMATION);
      }
      return 0;
    }
...
static void InitPatch() {
  ...
  // Backlog icon fix: hook after table is built to reverse-translate names
  if (g_fontManager.GetEnableBacklogAllIcon()) {
