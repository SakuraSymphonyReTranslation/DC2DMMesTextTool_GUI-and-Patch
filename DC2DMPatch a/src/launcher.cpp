...
#else
  sprintf_s(exePath, "%s\\DC2DM.EXE", currentDir);
  sprintf_s(dllPath, "%s\\DC2DMPatch.dll", currentDir);

  if (GetFileAttributesA(exePath) == INVALID_FILE_ATTRIBUTES) {
    MessageBoxA(NULL,
                "DC2DM.EXE not found!\nPlease place this launcher in the DC2DM "
                "game folder.",
                "DC2DM Patch Launcher", MB_ICONERROR);
    return 1;
  }
  if (GetFileAttributesA(dllPath) == INVALID_FILE_ATTRIBUTES) {
    MessageBoxA(NULL,
                "DC2DMPatch.dll not found!\nPlease place DC2DMPatch.dll in the "
                "game folder.",
                "DC2DM Patch Launcher", MB_ICONERROR);
    return 1;
  }
