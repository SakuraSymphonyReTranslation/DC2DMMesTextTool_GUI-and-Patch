...
static class Program
{
    ...
    static void PrintBanner()
    {
        ...
        Console.WriteLine("╔════════════════════════════════════════════════╗");
        Console.WriteLine("║       DC2DM MES Text Tool v1.2 (GUI+CLI)         ║");
    ...
    static int HandleExport(MesToolProcessor processor, string[] args)
    {
        if (args.Length < 1)
        {
            Console.WriteLine("Usage: DC2DMMesTextTool export <input.mes> [output.json]");
            return 1;
        }
    ...
    static int HandleImport(MesToolProcessor processor, string[] args)
    {
        if (args.Length < 2)
        {
            Console.WriteLine("Usage: DC2DMMesTextTool import <input.json> <original.mes> [output.mes]");
            return 1;
        }
    ...
    static int HandleBatchExport(MesToolProcessor processor, string[] args)
    {
        if (args.Length < 1)
        {
            Console.WriteLine("Usage: DC2DMMesTextTool batch-export <mes_folder> [json_folder]");
            return 1;
        }
    ...
    static int HandleBatchImport(MesToolProcessor processor, string[] args)
    {
        if (args.Length < 2)
        {
            Console.WriteLine("Usage: DC2DMMesTextTool batch-import <json_folder> <mes_folder> [output_folder] [-w <wrap_width>]");
            return 1;
        }
